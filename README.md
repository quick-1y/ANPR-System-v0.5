...
- **Коррекция наклона номера** — гибридный алгоритм выравнивания по линиям/контурам/углам с fallback до исходного кадра
...
- **Препроцессинг** — коррекция наклона и перспективы (Hough/контуры/базовая линия текста/углы), шумоподавление, бинаризация, ресайз до 128×32
...
flowchart LR
    ...
    M -->|Вне диапазона| Z
    D --> S[Коррекция наклона/перспективы номера]
    S --> E[Пакетный OCR CRNN]
...
- **Гибридная коррекция наклона** — быстрый отбор по линиям с fallback на контуры/углы и безопасным возвратом исходника
