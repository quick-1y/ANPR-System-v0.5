...
- **Настройки каналов** (с превью и редактором ROI):
  ...
  - Фильтр по размеру рамки номера с указанием мин/макс ширины/высоты и возможностью его отключить для свободной пропуска детекций
...
flowchart LR
    ...
    C --> M{Проверка мин/макс размера номера (если включено)}
...
- **Устойчивость к смене разрешения/ROI** — при пустых кадрах или изменении размера ROI детектор сбрасывает предыдущее состояние, чтобы не выполнять вычитание кадров разных размеров
...
└── anpr/                     # Основной пакет приложения
    ...
    │   ├── event_writer.py     # Асинхронная запись событий и скриншотов
...
### ♻️ Очистка неиспользуемого кода
- Удалены неиспользуемые методы управления метриками трекинга в `SettingsManager` и устаревшие вставки в `EventDatabase`, чтобы сократить публичный API конфигурации и хранилища.
...
1.  **Экспорт событий** — выгрузка детекций в CSV/Excel и отправка по webhook.
2.  **Мониторинг производительности** — встроенная панель с метриками задержек и загрузки ресурсов.
