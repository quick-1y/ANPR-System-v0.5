...
- **Схема таблицы**:
  ...
  - `channel` — канал, где был найден номер
  - `plate` — финальный номер после постобработки
  - `country_code` — код страны для отображения флага
...
- **Плагины стран** — YAML-файлы в `anpr/validation/configs` c регулярками форматов, валидными символами и стоп-словами
- **Коррекция OCR** — замена похожих символов (0→О, 3→З, латиница → кириллица) перед консенсусом трека
- **Фильтрация** — проверка длины, стоп-слов и запрещённых последовательностей перед записью события
- **Настройки** — выбор стран и директории шаблонов в разделе общих настроек UI
