...
- **Классификация** — APPROACHING (к камере), если доминирует вертикальное смещение вниз/рост рамки; RECEDING (от камеры), если смещение вверх/сжатие рамки. В интерфейсе направления показываются стрелками: `↓` — к камере, `↑` — от камеры.
...
- **Интеграция** — рассчитанное направление сохраняется вместе с событием, отображается в таблицах событий и в журнале поиска. В дебаг-режиме можно включить отрисовку стрелок трека (`show_direction_tracks`) поверх видеопотока.
