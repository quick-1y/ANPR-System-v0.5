...
- **Постобработка номеров** — коррекция OCR-опечаток, определение страны и фильтрация невалидных номеров по шаблонам
...
flowchart LR
    ...
    F -->|Достигнут кворум| V[Валидация / постобработка]
    F -->|Недостаточно данных| Z
    V -->|Корректный номер| G[Проверка кулдауна]
    V -->|Фильтрация| Z
...
### Валидация и форматы номеров
- **Конфигурации стран** — YAML-файлы в `configs/countries/*.yaml`
- **Коррекция OCR** — преобразование похожих символов (0→О, 1→І/Т и др.)
- **Фильтры** — стоп-слова, недопустимые последовательности и регэкспы форматов
- **Настройка** — в общих настройках выбирается каталог и перечень активных стран
