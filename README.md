...
- **Трекинг и агрегация** — отслеживание объектов с отбором бестшотов по качеству и голосованием по OCR
...
flowchart LR
    ...
    M -->|Вне диапазона| Z
    D --> Q[Оценка качества бестшотов\n(conf/резкость/экспозиция/размер/ракурс)]
    Q --> E[Пакетный OCR CRNN]
...
### Критерии отбора бестшотов
- **Confidence детекции** — минимальный порог для допуска кадра в пул.
- **Резкость** — измеряется по дисперсии лапласиана (отсев смазанных кадров).
- **Экспозиция** — контроль пересветов/недосветов по средней яркости.
- **Размер номера** — приоритет более крупным bbox (номер занимает значимую часть кадра).
- **Соотношение сторон** — штраф за сильные перспективные искажения.

Бестшоты выбираются *до* OCR, после чего выполняется пакетное распознавание и консенсусное решение по треку.
