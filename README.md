...
- **Постобработка номеров** — валидация по шаблонам стран, исправление типичных OCR-ошибок и фильтрация невалидных последовательностей
...
- **Батчевый OCR** — группировка кропов номеров в батчи для ускорения CRNN
- **Пул QPixmap** — повторное использование буферов и отрисовка через QPainter без постоянных аллокаций при масштабировании
...
flowchart LR
    ...
    F -->|Достигнут кворум| P[Валидация номера]
    F -->|Недостаточно данных| Z
    P -->|Неверный формат| Z
    P -->|Валиден| G[Проверка кулдауна]
    G -->|Кулдаун прошёл| H[Сохранение в БД]
    G -->|В кулдауне| Z
    H --> I[Обновление UI]
```

## ✅ Валидация и постобработка номеров

- **Конфигурации стран** — YAML-файлы в `anpr/postprocessing/configs` с регулярными выражениями форматов, допустимыми символами и правилами коррекции.
- **Коррекция OCR** — автоматические замены похожих символов (0→О, 1→Т/ I, 8→В/ B), конвертация латиницы в кириллицу для нужных стран и удаление лишних символов.
- **Фильтры качества** — проверка на стоп-слова, повторяющиеся/растущие последовательности и недопустимые символы.
- **Расширяемость** — добавьте новую страну, создав YAML-файл и отметив её в общих настройках приложения.
- **Точка включения** — постобработка выполняется после консенсуса по треку, чтобы не нагружать систему в процессе голосования.

### 🔎 Отладка распознавания
- В карточке события отображается журнал этапов: детектор, OCR, агрегация, постобработка и итоговое решение.
- Для каждого шага сохраняется пояснение (в том числе логи валидатора по конфигурациям стран).
...
- **Схема таблицы `events`**:
  ...
  - `plate` — нормализованный госномер
  - `raw_plate` — сырая строка OCR до валидации
  - `confidence` — уверенность OCR
  - `country` — ISO-код страны, определённой валидатором
  - `plate_format` — имя совпавшего шаблона
  - `frame_path` — путь к полному кадру
  - `plate_path` — путь к кропу номера
  - `debug_log` — JSON-массив шагов постобработки (для отображения в UI)
  - `source` — строка подключения к источнику
  - `confidence` — уверенность распознавания
  - `raw_plate` — исходная строка OCR до коррекции
  - `country` — код страны формата номера
  - `plate_format` — совпавший шаблон внутри страны
