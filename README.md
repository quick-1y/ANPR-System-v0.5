...
- **Трекинг и агрегация** — сбор лучших кадров по треку с оценкой качества и консенсусом OCR
...
- **Препроцессинг** — коррекция перспективы (поиск четырёхугольника), шумоподавление, бинаризация по Оцу
...
### Алгоритм мульти-кадрового OCR (best-shot)
1. **Детекция и трекинг**: YOLOv8 назначает `track_id` каждой рамке номера.
2. **Сбор кандидатов**: для каждого трека рассчитывается *оценка качества кадра* (sharpness, размер, яркость, геометрия, confidence детекции).
3. **Отбор best-shots**: в буфере трека хранится топ-N кадров с лучшим качеством.
4. **OCR батчем**: после набора N кадров или тайм-аута трека — OCR запускается **только на best-shots**.
5. **Агрегация**: результаты OCR голосуются (кворум + веса `confidence × quality`).
6. **Валидация/кулдаун**: постобработка и подавление дублей перед записью события.
...
flowchart LR
    ...
    M -->|Вне диапазона| Z
    D --> Q[Оценка качества кадра]
    Q --> R{Best-shots >= N или тайм-аут?}
    R -->|Да| E[OCR CRNN по best-shots]
    R -->|Нет| Z
...
├── settings.json             # Конфигурация приложения (автоматически создаётся)
│
