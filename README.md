...
- **Постобработка номеров** — коррекция OCR, определение страны по шаблонам и фильтрация невалидных комбинаций
...
- Определение страны по регулярным выражениям и приоритетам профилей (RU, BY, KZ, UA из коробки)
- Коррекция частых OCR-ошибок (0→О, 1→Т/І, 8→В и др.) с учётом специфики каждой страны
- Нормализация входных строк (удаление пробелов, дефисов, приведение регистра)
- Проверки стоп-слов, некорректных последовательностей и допустимых символов
- Расширяемость через YAML-конфиги в `anpr/plate_postprocessing/configs`
- Выбор активных шаблонов стран в общих настройках приложения
- Сохранение в БД нормализованного номера и кода страны для отображения флагов
...
- **База данных** — SQLite (`data/db/anpr.db` по умолчанию)
  - **Схема таблицы**:
    ...
    - `channel` — канал, на котором распознан номер
    - `plate` — финальный номер после постобработки
    - `confidence` — уверенность распознавания
    - `source` — источник видео
    - `frame_path` — путь к полному кадру
    - `plate_path` — путь к кропу номера
    - `country_code` — код страны (для показа флага)
    - `country_name` — отображаемое название страны
    - `normalized_plate` — очищенная/скорректированная версия номера
    - `format_name` — сработавший шаблон формата
