...
- **Валидация и постобработка** — нормализация OCR, коррекция частых ошибок и отбор по шаблонам стран
...
flowchart LR
    ...
    F -->|Достигнут кворум| V[Постобработка и валидация]
...
- **plate_countries** — список кодов стран, правила которых активны для валидации (настраивается в "Общих" настройках)
- **Плагины стран** — YAML-файлы в `anpr/postprocessing/configs`, позволяющие добавлять новые шаблоны без изменения кода
- **Коррекция OCR** — автоисправление похожих символов (0→O, 1→T/I, 8→B и др.), удаление пробелов/дефисов и фильтрация стоп-слов/последовательностей
...
- **Схема таблицы**:
  ...
  - `country` — определённый код страны (для отображения флага)
