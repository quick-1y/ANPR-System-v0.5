...
- **Постпроцессинг госномеров** — нормализация, коррекция OCR и валидация по шаблонам стран
...
flowchart LR
    ...
    F -->|Достигнут кворум| V[Валидация и постобработка]
    F -->|Недостаточно данных| Z
    V -->|Валидный номер| G[Проверка кулдауна]
    V -->|Отклонён| Z
...
### Валидация номеров
- **Плагины стран** — YAML-конфиги в `anpr/postprocessing/configs` с регэкспами форматов и правилами коррекции OCR
- **Коррекция OCR** — перевод цифр в похожие буквы, удаление пробелов/дефисов
- **Фильтры** — стоп-слова, повторяющиеся последовательности, проверка допустимых символов
- **Выбор стран** — на вкладке общих настроек можно включить/выключить шаблоны стран, чтобы не валидировать лишние регионы
...
- **Схема таблицы**:
  ...
  - `plate` — распознанный номер
  - `normalized_plate` — нормализованное значение после коррекции OCR
  ...
  - `source` — исходный поток
  - `country_code` — код страны по совпавшему шаблону
...
│   │   ├── __init__.py
│   │   ├── configs/            # YAML-шаблоны стран
│   │   ├── postprocessor.py    # Логика постобработки
│   │   └── registry.py         # Регистрация шаблонов стран
│   │
