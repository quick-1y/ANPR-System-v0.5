...
- **Редактор ROI** — многоугольник в пикселях с добавлением/удалением точек двойным кликом прямо по кадру
- **Отладочный режим каналов** — по желанию отображает рамки детекции YOLO и символы OCR поверх видеопотока
...
- **Схема таблицы**:
  ...
  - `country` — код страны (для вывода флага)
  - `confidence` — уверенность распознавания
  - `source` — источник видеопотока
