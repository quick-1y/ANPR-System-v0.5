...
class EventDatabase:
    ...
    def insert_event(
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                (
                    "INSERT INTO events (timestamp, channel, plate, confidence, source, frame_path, plate_path, country)"
                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                ),
                (ts, channel, plate, confidence, source, frame_path, plate_path, country),
            )
            conn.commit()
            self.logger.info(
