...
class YOLODetector:
    ...
    def __init__(self, model_path: str, device, confidence_threshold: float | None = None) -> None:
        self.model = YOLO(model_path)
        self.model.to(device)
        self.device = device
        self.confidence_threshold = float(confidence_threshold or Config().detection_confidence_threshold)
        self._tracking_supported = True
        logger.info(
            "Детектор YOLO успешно загружен (model=%s, device=%s, conf=%.2f)",
            model_path,
            device,
